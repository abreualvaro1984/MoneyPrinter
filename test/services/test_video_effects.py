...
def _gradient_clip(width=64, height=48, duration=1.0):
    """Crie gradientes não uniformes para garantir que as diferenças de pixels antes e depois do dimensionamento possam ser detectadas de forma confiável."""
...
def _detail_frame(width=128, height=96):
    """Crie quadros RGB contendo detalhes de alta frequência para observar se a escala de subpixel responde continuamente."""
...
class TestFadeAndSlideTransitions(unittest.TestCase):
    def test_fade_transitions_apply_requested_duration(self):
        """Fade in e fade out devem passar pela duração passada pelo chamador para o efeito MoviePy como ele está."""
        clip = _gradient_clip()
    ...
    def test_slidein_positions_cover_all_directions_and_unknown_side(self):
        """A animação deslizante deve permanecer estável em todas as quatro direções, na posição final e na parte inferior da direção desconhecida."""
    ...
    def test_slideout_positions_cover_timing_and_all_directions(self):
        """
        O slideout deve começar a se mover no final do clipe; quatro direções, horário de término passado e parâmetros de duração zero
        Todos precisam ser fixados para evitar divisão por zero ou saída antecipada de material.
        """
        clip = _gradient_clip(width=60, height=40, duration=2)
        self.addCleanup(clip.close)
        expected_ends = {
...
class TestZoomTransitions(unittest.TestCase):
    ...
    def test_zoomin_preserves_geometry_and_zooms_over_time(self):
        ...
        # A ampliação começa em 1x, portanto o primeiro quadro deve ser consistente com o original.
        np.testing.assert_allclose(first, original, atol=2)
        # O último quadro é de uma área central cortada e ampliada e deve ser significativamente diferente da imagem original.
    ...
    def test_zoomout_starts_zoomed_and_returns_to_source(self):
        ...
        # O primeiro quadro da redução é 1,2x e o final retorna exatamente à proporção original.
    ...
    def test_zoom_frame_responds_to_subpixel_scale_changes(self):
        ...
        # Essas duas proporções cairiam no mesmo tamanho de corte no antigo algoritmo de corte inteiro, produzindo exatamente o mesmo quadro,
        # Isto é seguido por um salto repentino ao cruzar um limite inteiro. A amostragem de subpixel deve ser capaz de responder a essas mudanças em pequena escala.
    ...
    def test_zoom_frame_keeps_center_stable_for_odd_resolution(self):
        ...
        # A largura e a altura ímpares têm apenas um pixel central exato, que não deve oscilar horizontal ou verticalmente após o dimensionamento.
        np.testing.assert_allclose(

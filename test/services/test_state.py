...
class TestMemoryState(unittest.TestCase):
    ...
    def test_patch_task_preserves_generated_outputs(self):
        """As atualizações de publicação assíncrona não podem substituir campos de tarefas de vídeo concluídos."""
        state = MemoryState()
        state.update_task(
            "task-1",
...
class TestRedisState(unittest.TestCase):
    ...
    def test_get_all_tasks_paginates_across_scan_batches(self):
        """
        Redis SCAN Ao retornar chaves em lotes, as fatias de paginação devem ser calculadas com base na posição inicial do lote atual.

        Reproduza este caso de uso PR #890 18 tarefas descritas,page_size=10 Cenário:
        10 itens no primeiro lote e 8 itens no segundo lote. A primeira página da lógica antiga retornará uma lista vazia e a segunda página retornará uma lista vazia.
        Apenas 2 itens são devolvidos; após o reparo, 10 itens são devolvidos na primeira página e os 8 itens restantes são devolvidos na segunda página.
        """
    ...
    def test_real_redis_get_all_tasks_ignores_queue_keys(self):
        """A fila List no Redis real não pode ser lida erroneamente como Hash pela lista de tarefas."""
        state = RedisState(
    ...
    def test_real_redis_patch_and_delete_are_atomic(self):
        """Exclusões simultâneas e atualizações parciais no Redis real não podem recriar tarefas incompletas."""
        state = RedisState(
            host=os.environ["MPT_TEST_REDIS_HOST"],
            port=int(os.getenv("MPT_TEST_REDIS_PORT", "6379")),
            db=int(os.getenv("MPT_TEST_REDIS_DB", "15")),
        ...
        for _ in range(50):
            ...
            # Future.result() A exceção do thread de trabalho será lançada novamente no thread de teste para evitar Redis
            # Na verdade, o comando falhou, mas apenas a exceção do thread foi impressa e acabou sendo mal avaliada como uma aprovação no teste.

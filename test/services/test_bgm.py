...
class TestBackgroundMusicService(unittest.TestCase):
    def test_should_use_bgm_is_provider_independent(self):
        """A mudança universal deve abranger as fontes atuais e os futuros fornecedores e não pode ser específica do fornecedor."""
        test_cases = [
            ("random", 0.2, True),
            ("custom", 0.2, True),
            ("sonilo", 0.2, True),
    ...
    def test_sanitize_upload_filename_strips_directory_components(self):
        self.assertEqual(
            bgm.sanitize_upload_filename("../../música do usuário.mp3"), "música do usuário.mp3"
        )
        self.assertEqual(
    ...
    def test_sanitize_upload_filename_accepts_common_audio_formats(self):
        # WebUI Compartilha a mesma lista de permissões de formato com a API; isso cobre maiúsculas e minúsculas e contêineres comuns para evitar futuras
        # Ao modificar o controle de upload, apenas a exibição da interface é suportada, mas a camada de serviço rejeita o arquivo.
    ...
    def test_save_bgm_upload_uses_atomic_storage_directory_write(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # O serviço irá restaurar o ponteiro do arquivo para o início e o mesmo UploadedFile ainda estará disponível Streamlit
            # Para audição ou uso subsequente de nova execução, ele não se tornará um arquivo vazio devido à operação de salvamento.
    ...
    def test_validate_bgm_upload_checks_audio_without_persisting_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # WebUI O Preflight permite apenas que arquivos temporários sejam usados ​​brevemente e o arquivo não pode ser salvo antes que o usuário clique em Gerar
            # Permanece no diretório BGM persistente e não pode alterar os ponteiros de arquivo necessários para escuta subseqüente.
    ...
    def test_validate_bgm_upload_rejects_invalid_audio_and_cleans_temp_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Uma simulação com falha também não pode deixar áudio temporário no diretório de persistência para evitar ser excluído aleatoriamente posteriormente. BGM
            # A lógica de enumeração é selecionada para evitar o acúmulo gradual de arquivos inválidos durante operações de longo prazo.
            self.assertEqual(os.listdir(temp_dir), [])
    ...
    def test_save_bgm_upload_rejects_invalid_audio_before_replacing_target(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # A falha na verificação ocorre antes da substituição atômica e os arquivos do usuário existentes não podem ser danificados.

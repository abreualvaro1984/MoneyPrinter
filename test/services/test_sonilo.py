...
class _StreamingResponse:
    """Fornece a interface mínima que requests.Response realmente usa nos serviços Sonilo."""
...
class TestSoniloService(unittest.TestCase):
    ...
    def test_request_timeout_clamps_fractional_and_invalid_values(self):
        """O tempo limite de leitura deve permanecer um número inteiro positivo aceito pelas Solicitações e limitar o tempo máximo de espera."""
        test_cases = [
            (0.5, (15, 1)),
            (1.1, (15, 2)),
            (1800.5, (15, 1800)),
    ...
    def test_connection_accepts_documented_hyphenated_service_id(self):
        """A escrita hifenizada em documentos públicos deve ser normalizada para o identificador de serviço interno do projeto."""
        response = _StreamingResponse(
    ...
    def test_connection_rejects_malformed_service_lists(self):
        """200 O sucesso da conexão não pode ser relatado ao WebUI em resposta a uma lista de serviços canônicos ausente."""
        invalid_payloads = [
            {},
    ...
    def test_connection_converts_network_and_invalid_json_errors(self):
        """Interrupções de rede e respostas não JSON para testes de conectividade são convertidas em exceções de domínio estável."""
        with (
            patch.object(sonilo.config, "app", {"sonilo_api_key": "test-key"}),
            patch.object(
                sonilo.requests,
    ...
    def test_create_video_proxy_uses_expected_ffmpeg_policy(self):
        """O proxy bem-sucedido deve cancelar o rastreamento, limitar o tamanho e fazer com que o chamador assuma a geração do arquivo."""
    ...
    def test_create_video_proxy_cleans_file_after_execution_failures(self):
        """FFmpeg Arquivos proxy ocultos não podem ser deixados para trás quando o tempo limite expirar, não forem executáveis ​​ou quando a codificação falhar."""
        failure_cases = [
            (
    ...
    def test_create_video_proxy_rejects_empty_and_oversized_outputs(self):
        """FFmpeg Para retornar com êxito, você também deve verificar novamente se o arquivo proxy existe, não está vazio e não excede o limite superior."""
    ...
    def test_stream_audio_rejects_error_empty_and_oversized_results(self):
        """Erros do lado do servidor, eventos somente de conclusão e volume excessivo de áudio não podem ser publicados como música de fundo válida."""
        ...
        for events, expected_message in cases:
            with (
                ...
                output = Path(temp_dir) / "music.m4a"
                # Todos os casos de uso reduzem uniformemente o limite superior de tamanho; eventos de erro e resultados vazios não são afetados por esse valor.
                # Os casos de uso acima do limite não exigem a alocação de 30 MB de dados no teste.
                with (
    ...
    def test_request_bgm_preserves_existing_output_and_cleans_temp_on_failures(self):
        """HTTP, as falhas na leitura de fluxo e na verificação de áudio não podem substituir os resultados existentes ou deixar produtos incompletos."""
        audio_event = _event(
            "audio_chunk",
            stream_index=0,
    ...
    def test_generate_bgm_converts_file_errors_and_cleans_proxy(self):
        """As falhas do sistema de arquivos também devem ser convertidas em exceções degradáveis ​​e quaisquer agentes gerados devem ser limpos."""
        with tempfile.TemporaryDirectory() as temp_dir:
            source = Path(temp_dir) / "source.mp4"
            proxy = Path(temp_dir) / "proxy.mp4"
            source.write_bytes(b"video")
    ...
    def test_generate_bgm_rejects_missing_key_and_input_before_proxy_work(self):
        """Deve falhar rapidamente quando faltam credenciais ou arquivos de entrada e não pode chamar FFmpeg ou APIs externas."""
        with tempfile.TemporaryDirectory() as temp_dir:
            source = Path(temp_dir) / "source.mp4"
            source.write_bytes(b"video")
            with (
